...
export function PromocodesAnalyticsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => analyticsApi.promocodes(params),
  });
  ...
  return (
    <Layout>
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Promocodes Analytics</h1>
      <DateFilter
        preset={preset}
        dateFrom={dateFrom}
        dateTo={dateTo}
              ...
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <Th col="discountPercent" label="Discount %" />
                    <Th col="usageCount" label="Usages" />
                    <Th col="revenue" label="Revenue" />
                    <Th col="uniqueUsers" label="Unique Users" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {(data?.data ?? []).map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600">{row.discountPercent}%</td>
                      <td className="px-4 py-3 text-sm text-slate-600">{row.usageCount}</td>
                      <td className="px-4 py-3 text-sm text-slate-600">{row.revenue?.toFixed(2)}</td>
                      <td className="px-4 py-3 text-sm text-slate-600">{row.uniqueUsers}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
